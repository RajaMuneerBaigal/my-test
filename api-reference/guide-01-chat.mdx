...
In this section, we will guide you on how to start chatting with your chatbot using the Chatsistant API.
...
To create a chat session, you need to use a POST request to the API endpoint: `https://app.chatsistant.com/api/v1/chatbot/{chatbot_uuid}/session/create`.
...
Here're example command to create a chatbot using the Chatsistant API:
...
```bash Curl
curl --location --request POST 'https://app.chatsistant.com/api/v1/chatbot/{chatbot_uuid}/session/create' \
...
url = 'https://app.chatsistant.com/api/v1/chatbot/{chatbot_uuid}/session/create'
headers = {
...
const url =
  "https://app.chatsistant.com/api/v1/chatbot/{chatbot_uuid}/session/create";
const headers = {
  "Content-Type": "application/json",
  Authorization: "Bearer <token>",
};
...
`https://app.chatsistant.com/api/v1/session/{session_uuid}/message/stream`
...
Here're example command to create a message using the Chatsistant API:

<Warning>Replace **token** with your actual API key.</Warning>

<CodeGroup>
    ```bash Curl
curl --location --request POST 'https://app.chatsistant.com/api/v1/session/{session_uuid}/message/stream' \
--header 'Content-Type: application/json' \
...
url = f"https://app.chatsistant.com/api/v1/session/{session_uuid}/message/stream"
headers = {
