...
<ParamField body="visibility" type="select">
 <p><span className="font-medium">Private</span>: No one can access your chatbot except you (your account).</p>
  <p><span className="font-medium">Public</span>: Anyone with the link can access it on <strong>chatsistant.com</strong>, and can be embedded on your website.</p>
...
<ParamField body="rate_limit" type="[number, number]">
  Limit the number of messages sent from one device on the iframe and chat bubble (this limit will not be applied to you on chatsistant, only on websites for your users to prevent abuse).
